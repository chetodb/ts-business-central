...
async function main() {
  const environment: BcEnvironment = 'Sandbox';
  const tenantId: string = 'dummy-tenant-id';
  const companyName: string = 'dummy-company';
  ...
  if (!tenantId || azureKeys.length === 0) {
    console.error(
      'Error: Missing Tenant ID or Azure Keys. Please complete the configuration before running the playground.',
    );
    return;
  }
  ...
  try {
    ...
    const items = await client.get<unknown[]>('item', { filter: filter, top: 5 });
